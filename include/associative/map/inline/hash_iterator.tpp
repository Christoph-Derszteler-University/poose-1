...
namespace containers::associative {
  ...
  hash_iterator<Bucket, Key, Value>::hash_iterator(
  ...
  template<typename Bucket, typename Key, typename Value>
  typename hash_iterator<Bucket, Key, Value>::value_type hash_iterator<Bucket, Key, Value>::operator*() const {
    const auto& tuple = ptr->at(outer_index).at(inner_index);
    return std::make_pair(std::get<0>(tuple), std::get<1>(tuple));
  }

  template<typename Bucket, typename Key, typename Value>
  hash_iterator<Bucket, Key, Value>& hash_iterator<Bucket, Key, Value>::operator++() {
  ...
  template<typename Bucket, typename Key, typename Value>
  hash_iterator<Bucket, Key, Value> hash_iterator<Bucket, Key, Value>::operator++(int) {
  ...
  template<typename Bucket, typename Key, typename Value>
  bool hash_iterator<Bucket, Key, Value>::operator==(const hash_iterator& other) const {
    return outer_index == other.outer_index
      && inner_index == other.inner_index;
  }

  template<typename Bucket, typename Key, typename Value>
  int hash_iterator<Bucket, Key, Value>::calculate_next_non_empty_bucket_index(
