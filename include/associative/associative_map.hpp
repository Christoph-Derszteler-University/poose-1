...
public:
   ...
   * @note If the key already exists, the value may be updated depending on the
   * implementation.
...
public:
  hashing_map(const std::function<hash_t(const Key &)> &hash_function,
...
public:
  hashing_multi_map(const std::function<hash_t(const Key &)> &hash_function,
                    const size_t &bucket_count);
  explicit hashing_multi_map(
      const std::function<hash_t(const Key &)> &hash_function);
